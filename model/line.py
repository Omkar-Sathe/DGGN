...
def create_model(numNodes, embedding_size, order='second'):
    ...
    first = Lambda(lambda x: reduce_sum(x[0] * x[1], axis=-1, keep_dims=False), name='first_order')([v_i_emb, v_j_emb])
    second = Lambda(lambda x: reduce_sum(x[0] * x[1], axis=-1, keep_dims=False), name='second_order')([v_i_emb_second, v_j_context_emb])
...
class LINE:
    def __init__(self, graph, embedding_size=8, negative_ratio=5, order='second', ):
        ...
        self.node_size = graph.number_of_nodes()    # 节点数量
        self.edge_size = graph.number_of_edges()    # 边的数量
        print("图的节点数量为{}，边的数量为{}".format(self.node_size, self.edge_size))
